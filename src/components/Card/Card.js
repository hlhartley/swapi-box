...
class Card extends Component {
    ...
    if(this.props.selected === 'favorites') {
        return <p className='card-body'>
           {}
        </p>
    }
    ...
    render() {
        ...
        return (
            <div className='card'>
                <h1 className='card-title'>{this.props.property.name}<button onClick={() => this.props.clickFavoriteButton(this.props.property)}><i className="far fa-star"></i></button></h1>
                {this.renderCards()}
            </div>
        )
    }
